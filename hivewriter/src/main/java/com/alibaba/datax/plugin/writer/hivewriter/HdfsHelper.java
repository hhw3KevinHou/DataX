...
public class HdfsHelper {
    ...
    //非常危险，如果路径错误，会删除父目录下所有表
    public void deleteDir(Path path){
        LOG.info(String.format("为保证安全，需要手动删除 tmp dir [%s] .",path.toString()));
//        try {
...
//        LOG.info(String.format("finish delete tmp dir [%s] .",path.toString()));
    ...
    public Class<? extends CompressionCodec> getCompressCodec(String compress) {
        ...
        if (null == compress || "NONE".equals(compress)) {
            codecClass = null;
    ...
    public void orcFileStartWrite(RecordReceiver lineReceiver, Configuration config, String fileName,
        ...
        try {
            ...
            while ((record = lineReceiver.getFromReader()) != null) {
                ...
                if (!transportResult.getRight()) {
                    //LOG.info("记录内容：{}",transportResult.getLeft());
                    ...
                    try {
                        switch (columnType) {
                            ...
                            case BOOLEAN:
                                ...
                                break;
                            case DATE:
                                Timestamp ts = new Timestamp(column.asDate().getTime());
                                org.apache.hadoop.hive.common.type.Date date =org.apache.hadoop.hive.common.type.Date.ofEpochMilli(
                                        ts.getTime());
                                recordList.add(date);
                                break;
                            case TIMESTAMP:
                                Timestamp ts1 = new Timestamp(column.asDate().getTime());
                                int nanos = ts1.getNanos();
                                org.apache.hadoop.hive.common.type.Timestamp time = org.apache.hadoop.hive.common.type.Timestamp.ofEpochMilli(
                                        ts1.getTime(), nanos);
                                recordList.add(time);
                                break;
                            default:
